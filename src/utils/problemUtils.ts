...
import {langExt,
		supportDebugLanguages} from '../shared';
import {isWindows,
		usingCmd} from './osUtils';
...
export async function getEntryFile(language: string, id: string): Promise<string> {
	...
	const specialCode: string = await getProblemSpecialCode(language, id, fileExt, extensionPath);
	const tmpEntryCode: string = (await fse.readFile(path.join(extensionPath, 'src/debug/entry', language, `entry.${fileExt}`))).toString();
